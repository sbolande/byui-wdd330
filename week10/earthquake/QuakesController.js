...
export default class QuakesController {
  ...
  async init() {
    ...
    console.log(`QuakesController initialized.`);
  }
  async initPos() {
    ...
    if (this.position.lat === 0) {
      try {
        ...
        if (!navigator.geolocation) throw Error('Geolocation is not supported by the browser');
        // if we get the location back then set the latitude and longitude into this.position
        else {
            navigator.geolocation.getCurrentPosition(geoPosition => {
                this.position.lat = geoPosition.coords.latitude;
                this.position.lon = geoPosition.coords.longitude;
            }, error => {
                throw Error('Unable to retrieve current location');
            });
        }
      } catch (error) {
        console.log(error);
